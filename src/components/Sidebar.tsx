import { useEffect, useState } from "react";
import type { Section } from "../types/section";

export default function Sidebar() {
  const [active, setActive] = useState<Section["id"]>("about");

  const sections: Section[] = [
    { id: "about", label: "SOBRE" },
    { id: "techs", label: "HABILIDADES" },
    { id: "experiences", label: "EXPERIÊNCIAS" },
    { id: "projects", label: "PROJETOS" },
    { id: "contact", label: "CONTATO" },
  ];

  function scrollToSection(id: string) {
    const el = document.getElementById(id);
    if (!el) return;
    window.scrollTo({ top: el.offsetTop, behavior: "smooth" });
  }

  useEffect(() => {
    function handleScroll() {
      for (const { id } of sections) {
        const el = document.getElementById(id);
        if (!el) continue;

        const rect = el.getBoundingClientRect();
        if (rect.top <= window.innerHeight * 0.4 && rect.bottom >= window.innerHeight * 0.3) {
          setActive(id);
          break;
        }
      }
    }

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  return (
    <div className="sticky top-0 h-auto md:h-screen py-12 md:py-24 px-6 md:px-20 flex flex-col justify-start md:justify-between gap-5">
      <div className="flex flex-col gap-3">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-300">Dryelle Ebelin</h1>
        <h3 className="font-bold text-gray-300">Desenvolvedora Front-end Web & Mobile</h3>
        <p>Transformo ideias em interfaces modernas, intuitivas e de alta performance.</p>
      </div>

      <nav className="flex flex-col items-start gap-3 hidden md:flex text-sm font-semibold">
        {sections.map(({ id, label }) => {
          const isActive = active === id;

          return (
            <button
              key={id}
              onClick={() => scrollToSection(id)}
              className={`
                text-left uppercase transition-colors hover:cursor-pointer
                ${isActive ? "text-indigo-500" : "hover:text-gray-300"}
              `}
            >
              {label}
            </button>
          );
        })}
      </nav>

      <div className="flex items-center gap-5 md:gap-4">
        <a
          className="hover:text-white transition-colors duration-300"
          href="https://github.com/dryelleebelin"
          target="_blank"
          rel="noopener noreferrer"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-github-icon lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" /><path d="M9 18c-4.51 2-5-2-7-2" /></svg>
        </a>
        <a
          className="hover:text-white transition-colors duration-300"
          href="https://www.linkedin.com/in/dryelleebelin"
          target="_blank"
          rel="noopener noreferrer"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-linkedin-icon lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" /><rect width="4" height="12" x="2" y="9" /><circle cx="4" cy="4" r="2" /></svg>
        </a>
        <a
          className="hover:text-white transition-colors duration-300"
          href="https://codepen.io/dryelleebelin"
          target="_blank"
          rel="noopener noreferrer"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-codepen-icon lucide-codepen"><polygon points="12 2 22 8.5 22 15.5 12 22 2 15.5 2 8.5 12 2" /><line x1="12" x2="12" y1="22" y2="15.5" /><polyline points="22 8.5 12 15.5 2 8.5" /><polyline points="2 15.5 12 8.5 22 15.5" /><line x1="12" x2="12" y1="2" y2="8.5" /></svg>
        </a>
        <a
          className="hover:text-white transition-colors duration-300"
          href="https://www.instagram.com/dryelleebelin"
          target="_blank"
          rel="noopener noreferrer"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-instagram-icon lucide-instagram"><rect width="20" height="20" x="2" y="2" rx="5" ry="5" /><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" /><line x1="17.5" x2="17.51" y1="6.5" y2="6.5" /></svg>
        </a>
      </div>
    </div>
  )
}